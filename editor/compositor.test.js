...
test('composite: draws circle and returns a composited JPEG blob for "click" stepType', async () => {
  const { composite } = await import('./compositor.js');
  ...
  assert.equal(strokeStyleCall.args[0], CONFIG.ANNOTATION.COLOR, 'color should match config');
});
