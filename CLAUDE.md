...
- **Annotated Screenshots**: The highlight circle annotation is stored as data alongside the raw screenshot. It is drawn dynamically as an overlay in the Editor and composited during export, rather than being baked into the image in the background service worker.
...
- Image compositing for exports happens in the Editor context (which has DOM access) rather than the background service worker, simplifying the background architecture.
